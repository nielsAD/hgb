...
uint32_t pagerank_bcsc_mpi_default(const pr_bcsc_graph_t *graph, pagerank_options_t *options)
{
    ...
    const graph_size_t *block_deg = graph->blocks_diag[block]->deg_i;
    ...
    while ((iterations < options->min_iterations) || (iterations < options->max_iterations && diff > options->epsilon))
    {
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            MPI_Allgather(
                block_src, max_block_len, MPI_PR_FLOAT,
                src[0],    max_block_len, MPI_PR_FLOAT,
                comm
            );
            ...
            for (graph_size_t col = 0; col < bcount; col++)
                omp_pagerank_update_rank_pull(
                    ...
                    graph->blocks[block*bcount+col]->deg_i,
...
uint32_t pagerank_bcsc_mpi_stepped(const pr_bcsc_graph_t *graph, pagerank_options_t *options)
{
    ...
    pr_float *restrict      _tmp = memory_talloc(pr_float, max_block_len*bcount);

    pr_float *restrict *restrict tmp = memory_talloc(pr_float*, bcount);
    for (graph_size_t b = 0; b < bcount; b++)
        tmp[b] = &_tmp[b*max_block_len];
    ...
    while ((iterations < options->min_iterations) || (iterations < options->max_iterations && diff > options->epsilon))
    {
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            omp_pagerank_update_tmp(tmp[block], block_src, block_deg, block_len);
            omp_pagerank_fill_arr(block_dst, 0.0, block_len);

            MPI_Allreduce(MPI_IN_PLACE, &base_rank, 1, MPI_PR_FLOAT, MPI_SUM, comm);
            MPI_Allgather(
                tmp[block], max_block_len, MPI_PR_FLOAT,
                tmp[0], max_block_len, MPI_PR_FLOAT,
                comm
            );
...
uint32_t pagerank_bcsc_mpi_mapped(const pr_bcsc_graph_t *graph, pagerank_options_t *options)
{
    ...
    while ((iterations < options->min_iterations) || (iterations < options->max_iterations && diff > options->epsilon))
    {
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            PAGERANK_TIME_START(BASERANK)
            omp_pagerank_baserank_mapped(tmp[block], block_src, block_deg, block_len);
            pr_float base_rank = omp_pagerank_sum_arr(tmp[block], block_len);
        ...
        PAGERANK_TIME_START(DIFF)
        omp_pagerank_calc_diff_mapped(tmp[block], block_src, block_dst, block_len);
        diff = omp_pagerank_sum_arr(tmp[block], block_len);
        MPI_Allreduce(MPI_IN_PLACE, &diff, 1, MPI_PR_FLOAT, MPI_SUM, comm);
        PAGERANK_TIME_STOP(DIFF)
...
uint32_t pagerank_bcsc_mpi_redux(const pr_bcsc_graph_t *graph, pagerank_options_t *options)
{
    ...
    const graph_size_t  block_len = graph->blocks_diag[row]->vcount;
    const graph_size_t *block_deg = graph->blocks_diag[row]->deg_i;
    ...
    while ((iterations < options->min_iterations) || (iterations < options->max_iterations && diff > options->epsilon))
    {
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            PAGERANK_TIME_START(UPDATE)
            MPI_Bcast(block_tmp, max_block_len, MPI_PR_FLOAT, col, comm_col);
            omp_pagerank_fill_arr(block_dst, 0.0, block_len);
            ...
            PAGERANK_TIME_START(TRANSFER)
            MPI_Reduce(MPI_SEND_RECV(block_dst, block_tmp), block_len, MPI_PR_FLOAT, MPI_SUM, row, comm_row);
            PAGERANK_TIME_STOP(TRANSFER)
