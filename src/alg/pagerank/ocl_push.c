...
uint32_t pagerank_bcsr_ocl_mapped(const pr_bcsr_graph_t *graph, pagerank_options_t *options)
{
    ...
    ocl_pr_problem_t *problem = ocl_pr_problem_new_bcsr(options->devid, graph);
    ...
    while ((iterations < options->min_iterations) || (iterations < options->max_iterations && diff > options->epsilon))
    {
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            for (graph_size_t block_col = 0; block_col < bcount; block_col++)
                ...
                {
                    cl_event events[2];
                    events[0] = problem->event_blocks[E_PR_PROBLEM_BLOCKS_DST][block_col];
                    events[1] = problem->event_blocks[E_PR_PROBLEM_BLOCKS_SRC][block_row];

                    ocl_pr_problem_set_event_b(problem,
                        E_PR_PROBLEM_BLOCKS_DST,
                        block_col,
                        ocl_pagerank_update_rank_push(
                            options->devid,
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_DST][block_col],
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_SRC][block_row],
                            ...
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_DEG][block_row],
        ...
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            for (graph_size_t block_col = 0; block_col < bcount; block_col++)
                for (graph_size_t block_row = 0; block_row < bcount; block_row++)
                {
                    cl_event events[2];
                    events[0] = problem->event_blocks[E_PR_PROBLEM_BLOCKS_DST][block_col];
                    events[1] = problem->event_blocks[E_PR_PROBLEM_BLOCKS_TMP_VTX][block_row];

                    ocl_pr_problem_set_event_b(problem,
                        E_PR_PROBLEM_BLOCKS_DST,
                        block_col,
                        ocl_pagerank_update_rank_tmp_push(
                            options->devid,
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_DST][block_col],
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_TMP_VTX][block_row],
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_RID][block_row*bcount+block_col],
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_CID][block_row*bcount+block_col],
                            graph->blocks[block_row*bcount+block_col]->vcount,
                            2, events
            ...
            for (graph_size_t block_col = 0; block_col < bcount; block_col++)
                for (graph_size_t block_row = 0; block_row < bcount; block_row++)
                {
                    ...
                    ocl_pr_problem_set_event_b(problem,
                        E_PR_PROBLEM_BLOCKS_DST,
                        block_col,
                        ocl_pagerank_update_rank_tmp_push_warp(
                            options->devid,
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_DST][block_col],
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_TMP_VTX][block_row],
                            problem->data_blocks[E_PR_PROBLEM_BLOCKS_RID][block_row*bcount+block_col],
