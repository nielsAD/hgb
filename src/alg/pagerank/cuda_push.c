...
uint32_t pagerank_csr_cud_lib(const pr_csr_graph_t *graph, pagerank_options_t *options)
{
    assert(graph != NULL);
    ...
    CUDA_ASSERT(cudaMemcpyAsync(degs, graph->deg_o,   sizeof(graph_size_t) *  vcount,      cudaMemcpyHostToDevice, NULL));
    ...
    cuda_pagerank_fill_cols_push(NULL, val, rows, cols, degs, vcount);
...
uint32_t pagerank_bcsr_cuda_mapped(const pr_bcsr_graph_t *graph, pagerank_options_t *options)
{
    ...
    PAGERANK_TIME_START(TRANSFER)
    cuda_pr_problem_t *problem = cuda_pr_problem_new_bcsr(graph);
    PAGERANK_TIME_STOP(TRANSFER)
        ...
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            for (graph_size_t block_col = 0; block_col < bcount; block_col++)
                for (graph_size_t block_row = 0; block_row < bcount; block_row++)
                    cuda_pagerank_update_rank_push(
                        problem->streams[block_col],
                        problem->data_blocks_f[E_PR_PROBLEM_BLOCKS_DST][block_col],
                        problem->data_blocks_f[E_PR_PROBLEM_BLOCKS_SRC][block_row],
                        ...
                        problem->data_blocks_u[E_PR_PROBLEM_BLOCKS_DEG][block_row],
...
uint32_t pagerank_bcsr_cuda_stepped(const pr_bcsr_graph_t *graph, pagerank_options_t *options)
{
    ...
    while ((iterations < options->min_iterations) || (iterations < options->max_iterations && diff > options->epsilon))
    {
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            for (graph_size_t block_col = 0; block_col < bcount; block_col++)
                for (graph_size_t block_row = 0; block_row < bcount; block_row++)
                    cuda_pagerank_update_rank_tmp_push(
                        problem->streams[block_col],
                        problem->data_blocks_f[E_PR_PROBLEM_BLOCKS_DST][block_col],
                        problem->data_blocks_f[E_PR_PROBLEM_BLOCKS_TMP_VTX][block_row],
                        problem->data_blocks_u[E_PR_PROBLEM_BLOCKS_RID][block_row*bcount+block_col],
                        problem->data_blocks_u[E_PR_PROBLEM_BLOCKS_CID][block_row*bcount+block_col],
                        graph->blocks[block_row*bcount+block_col]->vcount
                    );
...
uint32_t pagerank_bcsr_cuda_stepped_warp(const pr_bcsr_graph_t *graph, pagerank_options_t *options)
{
    ...
    while ((iterations < options->min_iterations) || (iterations < options->max_iterations && diff > options->epsilon))
    {
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            for (graph_size_t block_col = 0; block_col < bcount; block_col++)
                for (graph_size_t block_row = 0; block_row < bcount; block_row++)
                    cuda_pagerank_update_rank_tmp_push_warp(
                        problem->streams[block_col],
...
uint32_t pagerank_bcsr_cuda_stepped_dyn(const pr_bcsr_graph_t *graph, pagerank_options_t *options)
{
    ...
    while ((iterations < options->min_iterations) || (iterations < options->max_iterations && diff > options->epsilon))
    {
        for (uint32_t local_iterations = 0; local_iterations < options->local_iterations; local_iterations++)
        {
            ...
            for (graph_size_t block_col = 0; block_col < bcount; block_col++)
                for (graph_size_t block_row = 0; block_row < bcount; block_row++)
                    cuda_pagerank_update_rank_tmp_push_dyn(
                        problem->streams[block_col],
                        graph->blocks[block_row*bcount+block_col]->ecount / graph->blocks[block_row*bcount+block_col]->vcount,
                        problem->data_blocks_u[E_PR_PROBLEM_BLOCKS_TMP_SCR][block_col],
                        problem->data_blocks_f[E_PR_PROBLEM_BLOCKS_DST][block_col],
                        problem->data_blocks_f[E_PR_PROBLEM_BLOCKS_TMP_VTX][block_row],
                        problem->data_blocks_u[E_PR_PROBLEM_BLOCKS_RID][block_row*bcount+block_col],
                        problem->data_blocks_u[E_PR_PROBLEM_BLOCKS_CID][block_row*bcount+block_col],
